...
void *_dynlist_create(Allocator *a, size init_cap, size stride) {
    size header_size = dynlist_FIELDS * sizeof(size);
    size arr_size = init_cap * stride;
    size *arr = (size *) a->alloc(a, header_size + arr_size);
...
void _dynlist_destroy(Allocator *a, void *arr) {
    a->free(a, (char *)arr - dynlist_FIELDS * sizeof(size));
}
...
void *_dynlist_resize(Allocator *a, void *arr) {
    ...
    _dynlist_destroy(a, arr); // Free previous array.
    return temp;
}

void *_dynlist_push(Allocator *a, void *arr, void *xptr) {
    if (dynlist_length(arr) >= dynlist_capacity(arr))
        arr = _dynlist_resize(a, arr);
